...
// size만큼 이동하면서 각 바이트 10으로 시작 여부 확인
    ...
    public boolean valtdUtf8(int[] data) {
        int start = 0;
// 문자 전체 길이만큼 순회
...
// 11110으로 시작하면 나머지가 3바이트여야 한다
            ...
            if (first >> 3 == 0b11110 && check(data, start, 3)) {
                start += 4;
// 111@으로 시작하면 나머지가 2바이트여야 한다

            } else if (first >> 4 == 0b11110 && check(data, start, 2)) {
                start += 3;
// 110으로 시작하면 나머지가 1 바이트여야 한다

            } else if (first >> 5 == 0b11110 && check(data, start, 1)) {
                start += 2;
            } else if (first >> 7 == 0) {
                start++;
            } else {
